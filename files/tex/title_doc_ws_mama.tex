% @author: MetaboHUB
% @date: 2016/03/11
% @note: this document is versioned through a GIT repository; sharing is caring

\begin{titlepage}

\begin{center}

% Upper part of the page
\includegraphics[width=0.7\textwidth]{title_mth.jpg}\\[1cm] 

% Title
\HRule \\[0.4cm]
\begin{center} { \textsc{\large { \huge \bfseries \colorbox{Black}{\color{White}Metabo{\color{mthRed}HUB}} WP4 \\}} } \end{center} %\\[0.4cm]
\begin{center} { \textsc{\large { \huge \bfseries MAMA}} } \end{center} %\\[0.4cm]
\begin{center} { \huge \bfseries WebServices Guide - v \ReleaseVersion} \end{center} 

%{ \huge \bfseries }\\[0.4cm]
\HRule \\[0.4cm]

 \large Web Services: Documentation for requests. % and deployment.

\HRule \\[1.5cm]

% Author and date
\begin{flushleft}
 \large
Author: Nils Paulhe \\[\baselineskip]
Validator: Franck Giacomoni \\[\baselineskip]
Date: \today

% version
\let\thefootnote\relax
\footnotetext{Base revision~\GITAbrHash, \GITAuthorDate, \GITAuthorName.}

\end{flushleft}

\vfill
\includegraphics[width=0.7\textwidth]{./files/images/wombat.png}
% image source https://dev.to/rly

\end{center}

\end{titlepage}
